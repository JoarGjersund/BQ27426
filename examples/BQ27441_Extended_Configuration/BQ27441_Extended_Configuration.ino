...
void setup()
{
    ...
    if (!lipo.begin()) // begin() will return true if communication is successful
    {
        ...
        Serial.println("Error: Unable to communicate with BQ27426.");
        Serial.println("  Check wiring and try again.");
        Serial.println("  (Battery must be plugged into Battery Babysitter!)");
        while (1)
            ;
    }
    Serial.println("Connected to BQ27426!");
    ...
    if (lipo.itporFlag()) //write config parameters only if needed
    {
        ...
        /*
            Design Energy should be set to be Design Capacity × 3.7 if using the BQ27426-G1A or Design
            Capacity × 3.8 if using the BQ27426-G1B
        */
...
{
    // Read battery stats from the BQ27426-G1A
